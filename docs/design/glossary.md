...
| **Patient** | A biological individual, identified globally by `(dataset_id, patient_id)`. `patient_id` is an **arbitrary index within the dataset** (e.g. `p0001`); what it maps to is up to the ingester. |
