...
{scan_id}__src-{source_variant}__emb-{embedding_model_id}.h5
  /                         # root attributes
      ...
      embedding_dim
  /coords                   # (N, 2|4) int — x, y (, w, h) in the WSI frame
...
```text
key = (coords, patch_size, patch_resolution, embedding_model_id, source_variant)
```
...
The bundle symlinks or copies these per-scan HDF5 files. Patch geometry can additionally be exported as GeoJSON for TissUUmaps viewing — see [Outlines & geometry exports](outlines.md).
